...
object Carers {
  ...
  implicit def stringToOptionDate(x: String) = Some(Xmls.asDate(x))
  implicit def stringStringToCarers(x: Tuple2[String, String]) = CarersXmlSituation(World(x._1, new TestNinoToCis), Xmls.validateClaim(x._2))
  ...
  def interestingDates = Engine.folding[CarersXmlSituation, Option[DateTime], List[DateTime]]((acc, opt) => acc ::: opt.toList, List()).title("Interesting Dates").
    ...
    code((c: CarersXmlSituation) => Some(c.birthdate())).
  ...
  def main(args: Array[String]) {
    //   val ranges =  DateRanges.interestingDatesToDateRangesToBeProcessedTogether(interestingDates(("2010-3-8", "CL100119A")), DateRanges.sunday);
