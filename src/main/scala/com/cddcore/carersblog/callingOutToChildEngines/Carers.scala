package com.cddcore.carersblog.callingOutToChildEngines
...
object Carers {
  ...
  val engine = Engine.folding[CarersXmlSituation, Option[KeyAndParams], List[KeyAndParams]]((acc, opt) => acc ::: opt.toList, List()).title("Check Guard Conditions").
