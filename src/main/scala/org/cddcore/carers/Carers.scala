...
object Carers {
  ...
  val checkGuardConditions = Engine.folding[CarersXmlSituation, Option[ReasonAndAmount], ReasonsAndAmount](_ + _, ReasonsAndAmount(None, List())).
    ...
    childEngine("Customers under age 16 are not entitled to CA").
    ...
    childEngine("Hours1 - Customers with Hours of caring must be 35 hours or more in any one week").
    ...
    childEngine("Qualifying Benefit 3 - DP's without the required level of qualyfing benefit will result in the disallowance of the claim to CA.").
    ...
    childEngine("Residence 3- Customer who is not considered resident and present in GB is not entitled to CA.").
    ...
    childEngine("Presence 2- Customers who have restrictions on their immigration status will be disallowed CA.").
    ...
    childEngine("Full Time Eduction 2", "Customer in FTE 21 hours or more each week are not entitled to CA.").
  ...
  def main(args: Array[String]) {
    val (result, trace) = Engine.trace { () => engine.apply((World("2010-3-1"), Xmls.validateClaim("CL100116A"))) }
