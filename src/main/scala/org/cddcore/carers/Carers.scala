...
case class CarersXmlSituation(w: World, e: Elem) extends XmlSituation {
  ...
  lazy val ClaimantNinoNino = xml(e) \ "ClaimantData" \ "ClaimantNINO" \ string
...
object Carers {
  ...
  implicit def toSome(x: ReasonAndAmount) = Some(x)
  implicit def toList(x: ReasonAndAmount) = List(x)
  implicit def toReasonsAndAmount(x: Tuple2[Double, String]) = new ReasonsAndAmount(Some(x._1), List(KeyAndParams(x._2)))
  implicit def toReasonsAndAmount(x: String) = new ReasonsAndAmount(None, List(KeyAndParams(x)))
  ...
  val checkGuardConditions = Engine.folding[CarersXmlSituation, Option[ReasonAndAmount], ReasonsAndAmount](_ + _, ReasonsAndAmount(None, List())).
    code((c: CarersXmlSituation) => None).title("Check Guard Conditions").
    ...
    because((c: CarersXmlSituation) => c.underSixteen, "underSixteen").
    ...
    childEngine("High income").
    useCase("Employment", "Customer in paid employment exceeding GBP100 (after allowable expenses) per week is not entitled to CA.").
    ...
    useCase("Self employment", "Customer in Self employed work earning more than the prescribed limit of GBP100 per week (after allowable expenses) are not entitled to CA.").
    scenario((World("2010-3-1"), "CL100114A"), "CL114A-self employed earning too much").
    expected(ReasonAndAmount("carers.nettIncome.moreThan100PerWeek")).

    childEngine("Property Income").
    useCase("Sublet property", "Customers receiving payment for subletting their property for board and lodgings receiving more than the prescribed limit of GBP100 (after allowable expenses) will be disallowed for CA.").
    ...
    useCase("Rental income", "Customer receiving an Income from the renting of another property or land in the UK or abroad either their own name or a share in a partners profit is above GBP100 per week(after allowable expenses) is not entitled to CA.").
    scenario((World("2010-3-1"), "CL100116A"), "CL116A-income from renting").
    expected(ReasonAndAmount("carers.income.rental")).

    build

  val engine = Engine[CarersXmlSituation, ReasonsAndAmount].param((s: String) => CarersXmlSituation(World("2010-6-9"), XML.loadString(s)), "Validate Claim XML").
    code((c: CarersXmlSituation) => new ReasonsAndAmount(c.nettIncome, "carers.validClaim")).title("Validate Claim").

    useCase("All guard conditions should be passed").
    scenario((World("2010-6-7"), "CL100108A"), "CL100108A-restriction on immigration status").
    expected(new ReasonsAndAmount(None, List(
      KeyAndParams("carers.nettIncome.moreThan100PerWeek"),
      KeyAndParams("carers.claimant.restriction.immigrationStatus"),
      KeyAndParams("carers.claimant.notResident")))).
    because((c: CarersXmlSituation) => checkGuardConditions(c).reasons.size > 0).
    code((c: CarersXmlSituation) => checkGuardConditions(c)).
    ...
    expected((95.0d, "carers.validClaim")).
    because((c: CarersXmlSituation) => c.incomeOk).
    ...
    expected((95.0, "carers.validClaim")).
  ...
  def main(args: Array[String]) {
    val (result, trace) = Engine.trace { checkGuardConditions.apply((World("2010-3-1"), Xmls.validateClaim("CL100104A"))) }
    //    println("Result: " + result.value.get())
    println(trace.map((x) => Strings.oneLine(x.toString)).mkString("\n"))
